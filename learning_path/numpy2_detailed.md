...
* **Introducing arrays:**
    * **Explanation:** Explain what an array is in the context of NumPy (a grid of values, all of the same type, indexed by a tuple of non-negative integers). Contrast it with Python lists (heterogeneous, less efficient for numerical operations).
    * **Example:** Basic conceptualization of data organized in rows and columns, like a spreadsheet.
* **Your first NumPy array:**
    * **Explanation:** Introduce `np.array()`.
    * **Example:** `my_list = [1, 2, 3, 4]`, then `my_array = np.array(my_list)`. Show `print(my_array)`, `print(type(my_array))`.
* **Creating arrays from scratch:**
    * **Explanation:** Focus on `np.zeros()`, `np.ones()`, `np.full()`, `np.empty()`. Explain when each is useful (e.g., initializing an array of zeros for a computation, creating a placeholder array).
    * **Example:**
...
* **A range array:**
    * **Explanation:** Introduce `np.arange()` for creating arrays with a sequence of numbers, similar to Python's `range()`. Also, `np.linspace()` for evenly spaced numbers over a specified interval.
    * **Example:**
        * `range_array = np.arange(0, 10, 2)` (output: `[0, 2, 4, 6, 8]`)
        * `linspace_array = np.linspace(0, 1, 5)` (output: `[0.0, 0.25, 0.5, 0.75, 1.0]`)
* **Array dimensionality:**
    * **Explanation:** Explain "dimensions" (axes) and "rank" of an array. Show how `ndim` and `shape` attributes work.
    * **Example:**
...
* **3D array creation:**
    * **Explanation:** Extend the concept of `np.array()` to 3 dimensions, thinking of it as a "stack" of 2D arrays.
    * **Example:** `three_d_array = np.array([[[1, 2], [3, 4]], [[5, 6], [7, 8]]])`. Show its `shape` and `ndim`.
* **The fourth dimension:**
    * **Explanation:** Briefly explain the concept of higher dimensions, even if not commonly used in everyday examples, to solidify the abstract understanding. Mention image processing (batches of images, height, width, color channels) or video data as potential use cases.
...
* **Flattening and reshaping:**
    * **Explanation:** Explain `flatten()`, `ravel()`, and `reshape()`. Differentiate `flatten()` (returns a copy) and `ravel()` (returns a view). Emphasize `-1` in `reshape()` for automatic dimension calculation.
    * **Example:**
...
* **NumPy data types:**
    * **Explanation:** Discuss the importance of data types (e.g., `int32`, `float64`, `bool`). Explain how they affect memory usage and computational speed. Show `dtype` attribute.
    * **Example:**
...
* **The `dtype` argument:**
    * **Explanation:** Show how to explicitly set the data type during array creation using the `dtype` argument.
    * **Example:** `specified_dtype_array = np.array([1, 2, 3], dtype=np.float32)`
* **Anticipating data types:**
    * **Explanation:** Discuss common scenarios where NumPy infers data types and potential issues (e.g., mixing integers and floats results in floats, strings make everything strings).
    * **Example:**
...
* **A smaller sudoku game:**
    * **Explanation:** A fun, practical application of array creation and initial understanding.
...
* **Indexing and slicing arrays:**
    * **Explanation:** Explain 0-based indexing. Demonstrate single element access, and basic slicing `[start:end:step]`. Highlight that slicing returns a view, not a copy (by default).
    * **Example:**
...
* **Slicing and indexing trees:**
    * **Explanation:** Apply 2D slicing to a hypothetical dataset.
...
* **Stepping into 2D:**
    * **Explanation:** Elaborate on using `[row_slice, col_slice]` with steps.
    * **Example:** `matrix = np.arange(1, 17).reshape(4, 4)`
        * `matrix[::2, ::2]` (every other row, every other column)
* **Sorting trees:**
    * **Explanation:** Introduce `np.sort()` (returns a sorted copy) and `.sort()` method (sorts in-place). Explain sorting along axes.
    * **Example:**
...
* **Filtering arrays:**
    * **Explanation:** Explain boolean indexing (masking). Create a boolean array based on a condition and use it to select elements.
    * **Example:** `ages = np.array([20, 25, 15, 30, 18])`
        * `ages > 20` (returns `[False, True, False, True, False]`)
        * `ages[ages > 20]` (returns `[25, 30]`)
* **Filtering with masks:**
    * **Explanation:** Deep dive into how masks work with multi-dimensional arrays.
...
* **Creating arrays from conditions:**
    * **Explanation:** Combine filtering with assignments to create new arrays or modify existing ones based on conditions.
    * **Example:** `sales = np.array([100, 150, 75, 200, 120])`
        * `high_sales = sales[sales > 100]`
        * `sales[sales < 100] = 90` (update values)
* **Adding and removing data:**
    * **Explanation:** Introduce `np.append()`, `np.insert()`, `np.delete()`. Emphasize that these operations typically return a *new* array, as NumPy arrays are fixed-size.
    * **Example:** `arr = np.array([1, 2, 3])`
        * `np.append(arr, 4)`
        * `np.insert(arr, 1, 99)`
* **Compatible or not?**
    * **Explanation:** Introduce the concept of shape compatibility for operations like `np.append()` and `np.insert()`. Explain that dimensions must match or be compatible for the operation to succeed.
    * **Example:** Attempting to `np.append` a 1D array to a 2D array without specifying an axis.
* **Adding rows:**
    * **Explanation:** Use `np.append()` or `np.vstack()` with `axis=0` to add rows to a 2D array.
    * **Example:**
        * `matrix = np.array([[1, 2], [3, 4]])`
        * `new_row = np.array([[5, 6]])`
        * `matrix_with_new_row = np.append(matrix, new_row, axis=0)` or `np.vstack((matrix, new_row))`
* **Adding columns:**
    * **Explanation:** Use `np.append()` or `np.hstack()` with `axis=1` to add columns.
    * **Example:**
        * `matrix = np.array([[1, 2], [3, 4]])`
        * `new_col = np.array([[5], [6]])`
        * `matrix_with_new_col = np.append(matrix, new_col, axis=1)` or `np.hstack((matrix, new_col))`
* **Deleting with `np.delete()`:**
    * **Explanation:** Explain how to delete elements, rows, or columns using `np.delete()`, specifying the index and axis.
    * **Example:**
...
* **Summarizing data:**
    * **Explanation:** Introduce common aggregation functions: `sum()`, `mean()`, `median()`, `min()`, `max()`, `std()` (standard deviation), `var()` (variance). Explain the `axis` argument for aggregations across specific dimensions.
    * **Example:**
...
* **Sales totals:**
    * **Explanation:** Practical application of `sum()` or `np.sum()`.
...
* **Plotting averages:**
    * **Explanation:** Combine `mean()` with a conceptual link to data visualization (e.g., using Matplotlib, even if not directly coding the plot in NumPy).
...
* **Cumulative sales:**
    * **Explanation:** Introduce `np.cumsum()` for calculating cumulative sums along an axis.
    * **Example:** `daily_sales = np.array([50, 75, 60, 90, 110])`
        * `cumulative_daily_sales = np.cumsum(daily_sales)`
* **Vectorized operations:**
    * **Explanation:** Explain the core concept of vectorized operations in NumPy – applying operations element-wise without explicit loops, leading to significant speed improvements. Contrast with traditional Python loops.
    * **Example:**
        ...
        * `np.sqrt(arr1)`
* **Tax calculations:**
    * **Explanation:** Practical application of vectorized operations for financial calculations.
    * **Example:** `prices = np.array([10.50, 20.00, 5.75])`
        * `tax_rate = 0.08`
        * `prices_with_tax = prices * (1 + tax_rate)`
* **Projecting sales:**
    * **Explanation:** More complex vectorized calculations involving multiple arrays or conditions.
...
* **Vectorizing `.upper()`:**
    * **Explanation:** Explain `np.vectorize()` for applying a non-NumPy (scalar) function element-wise to a NumPy array. Emphasize that it's generally slower than native NumPy ufuncs but useful for string operations or custom functions.
    * **Example:**
        * `names = np.array(['alice', 'bob', 'charlie'])`
        * `uppercase_func = np.vectorize(str.upper)`
        * `uppercase_names = uppercase_func(names)` (output: `['ALICE', 'BOB', 'CHARLIE']`)
* **Broadcasting:**
    * **Explanation:** Explain the rules of broadcasting: when arrays have different shapes, NumPy attempts to "broadcast" the smaller array across the larger array so that they have compatible shapes. Rules: 1. If dimensions differ, prepend 1s to the smaller shape. 2. Compare dimension sizes. They must be equal, or one must be 1. 3. If a dimension is 1 in one array and greater than 1 in the other, the array with 1 is stretched.
    * **Example:** Conceptualize adding a 1D array to each row of a 2D array.
* **Broadcastable or not?**
    * **Explanation:** Provide examples of shapes that are compatible and incompatible for broadcasting, walking through the rules.
    * **Example:**
...
* **Broadcasting across columns:**
    * **Explanation:** Apply broadcasting rules to add a 1D array (representing a column) to each column of a 2D array. This often involves `reshape(-1, 1)` to make the 1D array a column vector.
    * **Example:**
        * `matrix = np.array([[1, 2, 3], [4, 5, 6]])`
        * `col_offset = np.array([10, 20]).reshape(-1, 1)` (becomes `[[10], [20]]`)
        * `result = matrix + col_offset`
* **Broadcasting across rows:**
    * **Explanation:** Apply broadcasting rules to add a 1D array (representing a row) to each row of a 2D array. This is often more straightforward.
    * **Example:**
        * `matrix = np.array([[1, 2, 3], [4, 5, 6]])`
...
* **Saving and loading arrays:**
    * **Explanation:** Introduce `np.save()` and `np.load()` for efficient binary storage of NumPy arrays (`.npy` files). Mention `np.savetxt()` and `np.loadtxt()` for text files (CSV, TSV), noting they are slower and less space-efficient for large arrays but human-readable.
    * **Example:**
        * `data_to_save = np.arange(100)`
        * `np.save('my_array.npy', data_to_save)`
* **Loading `.npy` files:**
    * **Explanation:** Show how to load previously saved `.npy` files.
    * **Example:** `loaded_data = np.load('my_array.npy')`
* **Getting help:**
    * **Explanation:** Demonstrate `np.info()` or `help(np.function_name)` for quick documentation access within a Python environment.
    * **Example:** `np.info(np.save)`
* **Update and save:**
    * **Explanation:** Combine loading, modifying an array, and then saving the updated version.
    * **Example:**
        * Load `sales_data.npy`.
        * Add a new month of sales or adjust existing values.
        * `np.save('updated_sales_data.npy', sales_data)`
* **Array acrobatics:**
    * **Explanation:** Broad overview of operations that change the arrangement of array elements without changing the data itself (e.g., flipping, transposing, rotating).
    * **Example:** Conceptually showing how an image might be rotated or mirrored.
* **Augmenting Monet:**
    * **Explanation:** Use image data (represented as a NumPy array of pixel values, typically `(height, width, channels)`) as a compelling real-world example for transformations.
...
* **Transposing your masterpiece:**
    * **Explanation:** Explain `np.transpose()` or the `.T` attribute for swapping axes.
    * **Example:**
...
* **Stacking and splitting:**
    * **Explanation:** Introduce `np.concatenate()`, `np.vstack()`, `np.hstack()`, `np.dstack()` for combining arrays, and `np.split()`, `np.vsplit()`, `np.hsplit()`, `np.dsplit()` for dividing arrays. Emphasize `axis` for concatenation/splitting.
    * **Example:**
...
* **2D split and stack:**
    * **Explanation:** Practical application of splitting and stacking 2D arrays.
...
* **Splitting RGB data:**
    * **Explanation:** Show how to split a 3D image array (height, width, channels) into separate arrays for each color channel (Red, Green, Blue).
    * **Example:**
...
* **Stacking RGB data:**
    * **Explanation:** Demonstrate reconstructing an image from individual color channels, useful after applying transformations or filters to individual channels.
    * **Example:**
